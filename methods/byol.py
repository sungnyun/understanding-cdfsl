...
class NetWrapper(nn.Module):
    def __init__(self, net, projection_size, projection_hidden_size, layer = -1):  # default layer = -2 since network includes classifier. Ours does not have classifier.
        super().__init__()
        self.net = net
        self.layer = layer
...
class BYOL(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert isinstance(net, BaselineTrain)
        self.net = net
        ...
        self.online_encoder = NetWrapper(net.feature, projection_size, projection_hidden_size, layer=hidden_layer)
        ...
        device = get_module_device(net.feature)
        self.to(device)
