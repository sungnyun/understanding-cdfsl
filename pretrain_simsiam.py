...
if __name__=='__main__':
    ...
    if params.method == 'baseline':
        ...
        model = BYOL(net=baseline, image_size=image_size, use_momentum=False) # SimSiam uses BYOL wrapper without momentum
    else:
        raise ValueError('Invalid `method` argument: {}'.format(params.method))

    if params.aug_mode is None:
        checkpoint_dir = '%s/checkpoints/%s/%s_%s_simsiam/type%s' %(configs.save_dir, params.dataset, params.model, params.method, str(params.pretrain_type))
    else:
        checkpoint_dir = '%s/checkpoints/%s/%s_%s_simsiam/type%s_%s' %(configs.save_dir, params.dataset, params.model, params.method, str(params.pretrain_type), params.aug_mode)
